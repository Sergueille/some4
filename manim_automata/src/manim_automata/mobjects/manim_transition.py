...
class ManimTransition(Transition, VGroup):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.transition_from == self.transition_to: #create transition that points to itself
            ...
            self.arrow = Arrow(start=transition_from.circle.get_center(), end=transition_to.circle.get_center(), buff=transition_from.circle.radius)
    ...
    def calculate_circle_vertices(self) -> tuple[int]:
        ...
        p1 = circle.point_at_angle(PI/2 + angle)
        p2 = circle.point_at_angle(PI/2 - angle)

        return p1, p2

    def create_reflexive_arrow(self, point1, point2) -> None:
        self.arrow = CurvedArrow(point2, point1, angle=1.3*PI)
    ...
    def position_text(self, buffer) -> None:
        ...
        if type(self.transition_from) == list:
            ...
            x1 = self.transition_from.circle.get_x()
            y1 = self.transition_from.circle.get_y()

        x2 = self.transition_to.circle.get_x()
        y2 = self.transition_to.circle.get_y()
    ...
    def construct_curved_arrow(self, buffer=0.5):
        ...
        from_point = self.transition_from.circle.get_center()
        to_point = self.transition_to.circle.get_center()

        from_to_vec = normalize(to_point - from_point)
        normal = rotate_vector(from_to_vec, math.pi / 2)

        angle = 0.8
        phi = angle / 2

        from_point += (from_to_vec * math.cos(phi) - normal * math.sin(phi)) * self.transition_from.circle.radius
        to_point -= (from_to_vec * math.cos(phi) + normal * math.sin(phi)) * self.transition_from.circle.radius
        
        self.arrow = CurvedArrow(from_point, to_point, angle=angle)
